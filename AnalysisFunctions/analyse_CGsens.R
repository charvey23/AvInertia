...
for(m in 1:36){
  ...
  for (l in 1:3){
    ...
    for (i in 1:length(shift)){
      ...
      diff_xnp_xcg                       = abs(dat_wing_curr$x_np_est_orgShoulder-(dat_sens_out$CGx[count]-dat_wing_curr$pt1_X)) #CHECK THIS
      dat_sens_out$prop_q_dot[count]     = (diff_xnp_xcg*dat_wing_curr$S_max*dat_wing_curr$full_m^0.24)/dat_sens_out$Iyy[count]
      dat_sens_out$prop_q_dot_nd[count]  = (diff_xnp_xcg*dat_wing_curr$S_max*dat_wing_curr$full_length^2)/dat_sens_out$Iyy[count]
      ...
      ## assume that a_t/a*(1-d(epsilon)/d(alpha))*eta_t = 0.8
      dat_sens_out$sm_TailNP[count]       = (dat_wing_curr$x_np_est_orgShoulder-dat_wing_curr$Vh*0.73*dat_wing_curr$c_mean_max)-(dat_sens_out$CGx[count]-dat_wing_curr$pt1_X)
...
for (j in 1:no_boot){
  ...
  for(i in 1:36){
    ...
    dat_sens_opt$max_stab[i] = dat_curr$sm[which(dat_curr$iter == iter_curr)]
...
for (j in 1:no_boot){
  ...
  for(i in 1:36){
    ...
    iter_curr  = sample(dat_curr$iter, 1, replace = FALSE, prob = NULL)
    dat_sens_opt$min_stab[i] = dat_curr$sm[which(dat_curr$iter == iter_curr)]
    dat_sens_opt$binomial[i] = dat_bird$binomial[i]
    dat_sens_opt$err[i]      = abs(shift[iter_curr])
  }
...
out_maxag <- pmc(tree = pruned_mcc, data = all_data_means_mat[,c("max_q_nd")], "BM", "OU", nboot = 5000, mc.cores = 1)
filename = paste(format(Sys.Date(), "%Y_%m_%d"),"_pmc_maxag_output.RData",sep="")
save(out_maxag,file = filename)
...
load("/Users/christinaharvey/Documents/AvInertia/AnalysisData/2021_08_19_pmc_xcg_output.RData")
...
load("/Users/christinaharvey/Documents/AvInertia/AnalysisData/2021_08_19_pmc_maxag_output.RData")
dists_maxag <- data.frame(null = out_maxag$null, test = out_maxag$test)
plot_maxag <- dists_maxag %>%
  gather(dists_maxag, value) %>%
  ggplot(aes(value, fill = dists_maxag)) +
  geom_density(alpha = 0.5) +
  geom_vline(xintercept = out_maxag$lr) +
  scale_fill_manual(values = c("null" = "black","test" = "#FFCC00"), name = "Model fit", labels = c("BM (null)","OU")) +
  # theme control
  th +
  # axis control
  scale_y_continuous(limits = c(0,1.3), name = "Density", breaks = c(0,0.5,1)) +
  scale_x_continuous(limits = c(-2,33), name = "Maximum likelihood estimates", breaks = c(0,10,20,30)) +
  geom_rangeframe() +
  annotate(geom = "segment", x = 0, xend = 30, y = log(0), yend = log(0))+
...
load("/Users/christinaharvey/Documents/AvInertia/AnalysisData/2021_08_19_pmc_maxstab_output.RData")
...
load("/Users/christinaharvey/Documents/AvInertia/AnalysisData/2021_08_19_pmc_minstab_output.RData")
...
load("/Users/christinaharvey/Documents/AvInertia/AnalysisData/2021_08_18_maxstab_z0.RData")
dat_sens_opt_max <- dat_sens_opt_out
load("/Users/christinaharvey/Documents/AvInertia/AnalysisData/2021_08_18_minstab_z0.RData")
...
boot_stab <- ggplot()+
  ...
  th +
  # axis control
  scale_y_continuous(limits = c(0,200), name = "Density", breaks = c(0,4,8,12)) +
  scale_x_continuous(limits = c(-0.06,0.06), name = expression(paste("Phenotypic optimum, ",theta))) +
  geom_rangeframe() +
  annotate(geom = "segment", x = -0.06, xend = 0.06, y = log(0), yend = log(0))+
  annotate(geom = "segment", y = 0, yend = 12, x = log(0), xend = log(0))

## ----------------------------------- Figure S3 -------------------------------------------------
load("/Users/christinaharvey/Documents/AvInertia/AnalysisData/2021_08_19_CGsensana.RData")

sens_agility <- ggplot()+
  geom_rect(data = shading, aes(ymin = col1, ymax = col2, xmin = 0.01, xmax = 20), alpha = 0.1, position = position_nudge(y = -0.5)) +
  ...
  th +
  scale_x_continuous(trans = "log10", limits = c(0.01,20), breaks = c(0.01,0.1,1,10),
                     labels = c(expression(10^-2),expression(10^-1),expression(10^0),expression(10^1)), name = expression(paste("Maximum pitch agility (s"^{-2},")"))) +
...
sens_agility_nd <- ggplot()+
  geom_rect(data = shading, aes(ymin = col1, ymax = col2, xmin = 0.01, xmax = 1.5), alpha = 0.1, position = position_nudge(y = -0.5)) +
  ...
  scale_colour_gradient2(low = "#D01B1B", mid = "white",high = "#95D2EC",midpoint = 0, name = "CG shift (%)", breaks = c(-0.15,-0.1,-0.05,0,0.05,0.1,0.15), labels = c(-15,-10,-5,0,5,10,15)) +
  th +
  scale_x_continuous(trans = "log10", limits = c(0.01,1.5), breaks = c(0.01,0.1,1,10),
                     labels = c(expression(10^-2),expression(10^-1),expression(10^0),expression(10^1)),  name = expression(paste("Normalized maximum pitch agility (s"^{-2},")"))) +
  scale_y_discrete(expand = c(0.1,0), limits = rev(phylo_order$species), name = "") +
  annotate(geom = "segment", x = 0.01, xend = 1, y = log(0), yend = log(0))
...
sens_stability_max <- ggplot()+
  ...
  geom_point(data = subset(dat_sens_out, group == "max_stability"), aes(y = species_order, x = sm, col = shift[iter], group = as.factor(BirdID)),
             position=position_dodge(width=0.5)) +
  geom_point(data = subset(dat_sens_out,iter==16 & group == "max_stability"), aes(y = species_order, x = sm, group = as.factor(BirdID)),
             fill = "white", col = "black", size = 1.5, pch = 23, position=position_dodge(width=0.5)) +
  geom_vline(xintercept = 0, col = "black") +
  scale_colour_gradient2(low = "#D01B1B", mid = "white",high = "#95D2EC",midpoint = 0, name = "CG shift (%)", breaks = c(-0.15,-0.1,-0.05,0,0.05,0.1,0.15), labels = c(-15,-10,-5,0,5,10,15)) +
  th +
  scale_x_continuous(limits = c(-0.15,0.15), name = "most aft static margin (m)") +
  scale_y_discrete(expand = c(0.1,0), limits = rev(phylo_order$species), name = "") +
  annotate(geom = "segment", x = -0.15, xend = 0.15, y = log(0), yend = log(0))

sens_stability_min <- ggplot()+
  geom_rect(data = shading, aes(ymin = col1, ymax = col2, xmin = -Inf, xmax = Inf), alpha = 0.1, position = position_nudge(y = -0.5)) +
  geom_point(data = subset(dat_sens_out, group == "min_stability"), aes(y = species_order, x = sm, col = shift[iter], group = as.factor(BirdID)),
             position=position_dodge(width=0.5)) +
  geom_point(data = subset(dat_sens_out,iter==16 & group == "min_stability"), aes(y = species_order, x = sm, group = as.factor(BirdID)),
             fill = "white", col = "black", size = 1.5, pch = 23, position=position_dodge(width=0.5)) +
  geom_vline(xintercept = 0, col = "black") +
  scale_colour_gradient2(low = "#D01B1B", mid = "white",high = "#95D2EC",midpoint = 0, name = "CG shift (%)", breaks = c(-0.15,-0.1,-0.05,0,0.05,0.1,0.15), labels = c(-15,-10,-5,0,5,10,15)) +
  th +
  scale_x_continuous(limits = c(-0.15,0.15), breaks = c(), name = "most forward static margin (m)") +
  scale_y_discrete(expand = c(0.1,0), limits = rev(phylo_order$species), name = "") +
  annotate(geom = "segment", x = -0.15, xend = 0.15, y = log(0), yend = log(0))
