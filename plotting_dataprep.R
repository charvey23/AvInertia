...
dat_rate_I     <- influ_continuous_BMmult(data=all_data_means_mat[,c("max_Ixx","max_Iyy","max_Izz")], phy=pruned_mcc, method="simulation",gp=gp.end,iter=999)
...
#dat_rate_I_sp  <- influ_continuous_BMmult(data=all_data_means_mat[,c("max_Ixx_specific","max_Iyy_specific","max_Izz_specific")], phy=pruned_mcc, method="simulation",gp=gp.end,iter=999)
...
OU_outputs = mvOU(tree = pruned_mcc, data = all_data_means_mat[,c("mean_CGx_specific_orgShoulder")])
exp(OU_outputs$theta)
...
tmp1               = aggregate(list(c_l_max = 0.25*dat_final$chord/-(dat_final$full_CGx-dat_final$pt1_X)), by = list(species = dat_final$species, BirdID = dat_final$BirdID), max)
tmp2               = aggregate(list(c_l_min = 0.25*dat_final$chord/abs(dat_final$full_CGx-dat_final$pt1_X)), by = list(species = dat_final$species, BirdID = dat_final$BirdID), min)
